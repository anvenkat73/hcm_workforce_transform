 SELECT  ASG.ASSIGNMENTID AS ASSIGNMENT_ID                  , 
  ...
  CASE WHEN MIN(PARSE_DATE('%Y-%m-%d',ASG.EFFECTIVESTARTDATE))> PARSE_DATE('%Y-%m-%d', '{{ var("VAR_PARAM_GLOBAL_INITIAL_EXTRACT_DATE") }}') THEN MIN(PARSE_DATE('%Y-%m-%d',ASG.EFFECTIVESTARTDATE)) ELSE PARSE_DATE('%Y-%m-%d', '{{ var("VAR_PARAM_GLOBAL_INITIAL_EXTRACT_DATE") }}') END AS EFFECTIVE_START_DATE           , 
