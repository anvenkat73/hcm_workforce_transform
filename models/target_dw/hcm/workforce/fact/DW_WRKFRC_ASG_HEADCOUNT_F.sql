 SELECT  HDC.ASSIGNWORKMEASUREID AS ASSIGN_WORK_MEASURE_ID , 
  ...
  CASE WHEN PARSE_DATE('%Y-%m-%d', '{{ var("VAR_PARAM_GLOBAL_INITIAL_EXTRACT_DATE") }}') > PARSE_DATE('%Y-%m-%d', HDC.EFFECTIVESTARTDATE ) THEN PARSE_DATE('%Y-%m-%d', '{{ var("VAR_PARAM_GLOBAL_INITIAL_EXTRACT_DATE") }}') ELSE PARSE_DATE('%Y-%m-%d', HDC.EFFECTIVESTARTDATE) END AS DW_DATE_FROM           , 
  HDC.EFFECTIVEENDDATE AS DW_DATE_TO             , 
  CASE WHEN PARSE_DATE('%Y-%m-%d', '{{ var("VAR_PARAM_GLOBAL_INITIAL_EXTRACT_DATE") }}') > PARSE_DATE('%Y-%m-%d', HDC.EFFECTIVESTARTDATE)  THEN 1 ELSE 0 END  AS DW_INITIAL_RECORD_IND  
