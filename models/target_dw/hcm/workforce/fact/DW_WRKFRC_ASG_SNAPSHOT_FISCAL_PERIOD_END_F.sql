 ...
 FROM  {{ ref('DW_CONTENT_PARAM_CONFIG') }} CFG INNER JOIN {{ ref('DW_FISCAL_DAY_D') }} PRD  ON CFG.PARAMETER_VALUE = PRD.FISCAL_PERIOD_SET_NAME 
 INNER JOIN  {{ ref('DW_WRKFRC_ASG_SNAPSHOT_F') }} FACT  ON (CASE WHEN PRD.LAST_DAY_FISCAL_PERIOD_FLAG = 'Y' THEN PRD.FISCAL_PERIOD_END_DATE ELSE PRD.FISCAL_DAY_DATE END) = FACT.CALENDAR_DATE
