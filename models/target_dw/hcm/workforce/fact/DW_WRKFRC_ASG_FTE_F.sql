 SELECT  FTE.ASSIGNWORKMEASUREID AS ASSIGN_WORK_MEASURE_ID , 
  ...
  CASE WHEN PARSE_DATE('%Y-%m-%d', '$VAR_PARAM_GLOBAL_INITIAL_EXTRACT_DATE$') > PARSE_DATE('%Y-%m-%d', FTE.EFFECTIVESTARTDATE ) THEN PARSE_DATE('%Y-%m-%d', '$VAR_PARAM_GLOBAL_INITIAL_EXTRACT_DATE$') ELSE PARSE_DATE('%Y-%m-%d', FTE.EFFECTIVESTARTDATE) END AS DW_DATE_FROM, 
  FTE.EFFECTIVEENDDATE AS DW_DATE_TO             , 
  CASE WHEN PARSE_DATE('%Y-%m-%d', '$VAR_PARAM_GLOBAL_INITIAL_EXTRACT_DATE$') > PARSE_DATE('%Y-%m-%d', FTE.EFFECTIVESTARTDATE)  THEN 1 ELSE 0 END  AS DW_INITIAL_RECORD_IND  
