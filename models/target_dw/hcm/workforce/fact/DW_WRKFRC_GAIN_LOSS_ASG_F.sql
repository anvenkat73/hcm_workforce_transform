 ...
 TO_CHAR(ASGEVTRSNL.ASSIGNMENT_ID)||'~'||TO_CHAR(ASGEVTRSNL.EVENT_DATE)||'~'||TO_CHAR(ASGEVTRSNL.EVENT_SEQUENCE)||'~'||TO_CHAR(1) AS SOURCE_RECORD_ID, 
 0 AS GAIN_IND, 
 1 AS LOSS_IND, 
 ASGEVTRSNL.EVENT_DATE-1 AS GAIN_LOSS_DATE, 
 ...
 ASGEVTRSNL.PRV_EVT_FULL_PART_TIME_FLAG AS FULL_PART_TIME_FLAG, 
  TRUNC(MONTHS_BETWEEN(ASGEVTRSNL.EVENT_DATE-1 , PERL.DATE_OF_BIRTH) / 12) AS AGE_IN_YEARS               , 
  CASE WHEN SPL.ACTUAL_TERMINATION_DATE IS NOT NULL AND SPL.ACTUAL_TERMINATION_DATE <= ASGEVTRSNL.EVENT_DATE-1  THEN TRUNC(MONTHS_BETWEEN(SPL.ACTUAL_TERMINATION_DATE , SPL.DATE_START)/12)   ELSE TRUNC(MONTHS_BETWEEN(ASGEVTRSNL.EVENT_DATE-1 , SPL.DATE_START)/12)   END AS TENURE_IN_YEARS            , 
  TRUNC(MONTHS_BETWEEN(ASGEVTRSNL.EVENT_DATE-1 , PERL.DATE_OF_BIRTH)) AS AGE_IN_MONTHS              , 
  CASE WHEN SPL.ACTUAL_TERMINATION_DATE IS NOT NULL AND SPL.ACTUAL_TERMINATION_DATE <= ASGEVTRSNL.EVENT_DATE-1  THEN TRUNC(MONTHS_BETWEEN(SPL.ACTUAL_TERMINATION_DATE , SPL.DATE_START))   ELSE TRUNC(MONTHS_BETWEEN(ASGEVTRSNL.EVENT_DATE-1 , SPL.DATE_START))   END AS TENURE_IN_MONTHS           , 
 ...
 FROM  {{ ref('DW_WRKFRC_ASG_EVENT_RSN_F') }} ASGEVTRSNL LEFT OUTER JOIN {{ ref('DW_PERSON_D') }} PERL  ON ( ASGEVTRSNL.PERSON_ID = PERL.PERSON_ID ) 
 LEFT OUTER JOIN  {{ ref('DW_SERVICE_PERIOD_D') }} SPL  ON ( ASGEVTRSNL.PERIOD_OF_SERVICE_ID = SPL.PERIOD_OF_SERVICE_ID AND ASGEVTRSNL.LEGISLATION_CODE = SPL.LEGISLATION_CODE ) LEFT OUTER JOIN  {{ ref('DW_WRKFRC_ASG_SUP_CHANGE_STAGE') }} SUPL  ON ( ASGEVTRSNL.ASSIGNMENT_ID = SUPL.ASSIGNMENT_ID AND (ASGEVTRSNL.EVENT_DATE-1) BETWEEN SUPL.EFFECTIVE_START_DATE AND SUPL.EFFECTIVE_END_DATE ) LEFT OUTER JOIN  {{ ref('DW_WRKFRC_ASG_HDC_CHANGE_STAGE') }} HDCL  ON ( ASGEVTRSNL.ASSIGNMENT_ID = HDCL.ASSIGNMENT_ID AND (ASGEVTRSNL.EVENT_DATE-1) BETWEEN HDCL.EFFECTIVE_START_DATE AND HDCL.EFFECTIVE_END_DATE ) LEFT OUTER JOIN  {{ ref('DW_WRKFRC_ASG_FTE_CHANGE_STAGE') }} FTEL  ON ( ASGEVTRSNL.ASSIGNMENT_ID = FTEL.ASSIGNMENT_ID AND (ASGEVTRSNL.EVENT_DATE-1) BETWEEN FTEL.EFFECTIVE_START_DATE AND FTEL.EFFECTIVE_END_DATE ) 
 WHERE  (ASGEVTRSNL.TERM_EVENT_IND=1 OR ASGEVTRSNL.JOB_CHANGE_IND =1 OR ASGEVTRSNL.GRADE_CHANGE_IND =1 OR  ASGEVTRSNL.LOCATION_CHANGE_IND =1 OR ASGEVTRSNL.POSITION_CHANGE_IND =1 OR ASGEVTRSNL.DEPARTMENT_CHANGE_IND =1 OR  ASGEVTRSNL.MANAGER_CHANGE_IND =1 OR ASGEVTRSNL.HEADCOUNT_CHANGE_IND =1 OR ASGEVTRSNL.FTE_CHANGE_IND =1 OR ASGEVTRSNL.BUSINESS_UNIT_CHANGE_IND=1 OR ASGEVTRSNL.FULL_PART_TIME_CHANGE_IND=1 OR ASGEVTRSNL.GLB_TRANSFER_OUT_IND=1 OR ASGEVTRSNL.ACTION_TYPE_CODE IN ('EMPL_END_ASG','EMPL_END_PROBATION','EMPL_END_TEMP_ASG','EMPL_END_TERMS','ORA_EMPL_CANCEL_WR') OR ASGEVTRSNL.ASG_END_OTHERS_IND=1 ) AND ASGEVTRSNL.EVENT_DATE-1 >= TO_DATE(SUBSTR('$VAR_PARAM_GLOBAL_INITIAL_EXTRACT_DATE$',1,19),'YYYY-MM-DD HH24:MI:SS')
