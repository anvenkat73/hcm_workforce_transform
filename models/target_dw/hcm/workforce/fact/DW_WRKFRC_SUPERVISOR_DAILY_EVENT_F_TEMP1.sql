 ...
 FROM  {{ ref('DW_WRKFRC_ASG_DAILY_EVENT_F') }} ASG_EVT2 INNER JOIN {{ ref('DW_MANAGER_DN_DH') }} DW_MANAGER  ON ( ASG_EVT2.ASSIGNMENT_ID = DW_MANAGER.ASSIGNMENT_ID AND ASG_EVT2.EVENT_DATE >= PARSE_DATE('%Y-%m-%d',DW_MANAGER.EFFECTIVE_START_DATE) AND DW_MANAGER.PRIMARY_MANAGER_FLAG = 'Y' AND ASG_EVT2.MANAGER_CHANGE_IND = 1 ) 
