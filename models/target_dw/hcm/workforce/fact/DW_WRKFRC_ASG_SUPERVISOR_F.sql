 SELECT  ASSIGNMENTSUPERVISORPVO.ASSIGNMENTSUPERVISORID AS ASSIGNMENT_SUPERVISOR_ID  , 
  ...
  CASE WHEN PARSE_DATE('%Y-%m-%d', '{{ var("VAR_PARAM_GLOBAL_INITIAL_EXTRACT_DATE") }}') > PARSE_DATE('%Y-%m-%d', ASSIGNMENTSUPERVISORPVO.EFFECTIVESTARTDATE ) THEN PARSE_DATE('%Y-%m-%d', '{{ var("VAR_PARAM_GLOBAL_INITIAL_EXTRACT_DATE") }}') ELSE PARSE_DATE('%Y-%m-%d', ASSIGNMENTSUPERVISORPVO.EFFECTIVESTARTDATE) END AS DW_DATE_FROM              , 
  ASSIGNMENTSUPERVISORPVO.EFFECTIVEENDDATE AS DW_DATE_TO, 
  CASE WHEN PARSE_DATE('%Y-%m-%d', '{{ var("VAR_PARAM_GLOBAL_INITIAL_EXTRACT_DATE") }}') > PARSE_DATE('%Y-%m-%d', ASSIGNMENTSUPERVISORPVO.EFFECTIVESTARTDATE)  THEN 1 ELSE 0 END AS DW_INITIAL_RECORD_IND
