 ...
 CASE WHEN PARSE_DATE('%Y-%m-%d', '{{ var("VAR_PARAM_GLOBAL_INITIAL_EXTRACT_DATE") }}') > PARSE_DATE('%Y-%m-%d', PERFMNCVO.DATEFROM ) THEN PARSE_DATE('%Y-%m-%d', '{{ var("VAR_PARAM_GLOBAL_INITIAL_EXTRACT_DATE") }}') ELSE PARSE_DATE('%Y-%m-%d', PERFMNCVO.DATEFROM) END AS DW_DATE_FROM,
 COALESCE(PARSE_DATE('%Y-%m-%d',PERFMNCVO.DATETO),DATE('4712-12-31')) AS DW_DATE_TO, 
 CASE WHEN PARSE_DATE('%Y-%m-%d', '{{ var("VAR_PARAM_GLOBAL_INITIAL_EXTRACT_DATE") }}') > PARSE_DATE('%Y-%m-%d', PERFMNCVO.DATEFROM)  THEN 1 ELSE 0 END AS DW_INITIAL_RECORD_IND
