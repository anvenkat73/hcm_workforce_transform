 ...
 COALESCE(PARSE_DATE('%Y-%m-%d',ASGPVO.ASSIGNMENTPEORETIREMENTDATE),DATE('1901-01-01')) AS RETIREMENT_DATE, 
  CASE WHEN PARSE_DATE('%Y-%m-%d', '{{ var("VAR_PARAM_GLOBAL_INITIAL_EXTRACT_DATE") }}') > PARSE_DATE('%Y-%m-%d', ASGPVO.EFFECTIVESTARTDATE ) THEN PARSE_DATE('%Y-%m-%d', '{{ var("VAR_PARAM_GLOBAL_INITIAL_EXTRACT_DATE") }}') ELSE PARSE_DATE('%Y-%m-%d', ASGPVO.EFFECTIVESTARTDATE) END AS DW_DATE_FROM, 
 ASGPVO.EFFECTIVEENDDATE AS DW_DATE_TO, 
 CASE WHEN PARSE_DATE('%Y-%m-%d', '{{ var("VAR_PARAM_GLOBAL_INITIAL_EXTRACT_DATE") }}') > PARSE_DATE('%Y-%m-%d', ASGPVO.EFFECTIVESTARTDATE)  THEN 1 ELSE 0 END  AS DW_INITIAL_RECORD_IND, 
