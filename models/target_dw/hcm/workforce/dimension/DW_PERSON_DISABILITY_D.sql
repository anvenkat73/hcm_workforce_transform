 SELECT  	PERSONDISABILITY.DISABILITYID AS PERSON_DISABILITY_ID	 , 
  ...
  CASE WHEN PARSE_DATE('%Y-%m-%d', '$VAR_PARAM_GLOBAL_INITIAL_EXTRACT_DATE$') > PARSE_DATE('%Y-%m-%d', PERSONDISABILITY.EFFECTIVESTARTDATE ) THEN PARSE_DATE('%Y-%m-%d', '$VAR_PARAM_GLOBAL_INITIAL_EXTRACT_DATE$') ELSE PARSE_DATE('%Y-%m-%d', PERSONDISABILITY.EFFECTIVESTARTDATE) END AS DW_DATE_FROM , 
  PERSONDISABILITY.EFFECTIVEENDDATE AS DW_DATE_TO , 
  CASE WHEN PARSE_DATE('%Y-%m-%d', '$VAR_PARAM_GLOBAL_INITIAL_EXTRACT_DATE$') > PARSE_DATE('%Y-%m-%d', PERSONDISABILITY.EFFECTIVESTARTDATE)  THEN 1 ELSE 0 END AS DW_INITIAL_RECORD_IND 
