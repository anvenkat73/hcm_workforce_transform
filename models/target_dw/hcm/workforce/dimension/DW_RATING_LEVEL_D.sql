 ...
 COALESCE(CAST(RCATPVO.RATINGCATEGORIESTRANSLATIONPEOCATEGORYID AS INT64),-99999) AS CATEGORY_ID, 
 CAST(RLEVPVO.RATINGLEVELBPEORATINGLEVELID AS STRING) ||'~'|| CAST(RLEVPVO.RATINGLEVELBPEORATINGMODELID  AS STRING) ||'~'||COALESCE(RCATPVO.RATINGCATEGORIESTRANSLATIONPEOCATEGORYID,-99999) AS SOURCE_RECORD_ID, 
 ...
 LEFT OUTER JOIN  {{ ref('stg_h_hcmprofilecontentlibraryam_ratingcategoriesfirstpvo') }} RCATPVO  ON 	(RLEVPVO.RATINGLEVELBPEORATINGMODELID=RCATPVO.RATINGCATEGORIESTRANSLATIONPEOCATEGORYID AND RLEVPVO.RATINGLEVELBPEOBUSINESSGROUPID=RCATPVO.RATINGCATEGORIESTRANSLATIONPEOBUSINESSGROUPID) 
