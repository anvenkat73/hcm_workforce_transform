 ...
 MHCF.PERSONID||'~'||MHCF.ASSIGNMENTID||'~'||FORMAT_DATE('%Y%m%d', PARSE_DATE('%Y-%m-%d', MHCF.EFFECTIVESTARTDATE)) ||'~'|| FORMAT_DATE('%Y%m%d', PARSE_DATE('%Y-%m-%d', MHCF.EFFECTIVEENDDATE))||'~'||MHCF.MANAGERTYPE||'~'||MHCF.ENTERPRISEID AS SOURCE_RECORD_ID, 
 ...
 CASE WHEN PARSE_DATE('%Y-%m-%d', '{{ var("VAR_PARAM_GLOBAL_INITIAL_EXTRACT_DATE") }}') > PARSE_DATE('%Y-%m-%d', MHCF.EFFECTIVESTARTDATE ) THEN PARSE_DATE('%Y-%m-%d', '{{ var("VAR_PARAM_GLOBAL_INITIAL_EXTRACT_DATE") }}') ELSE PARSE_DATE('%Y-%m-%d', MHCF.EFFECTIVESTARTDATE) END  AS DW_DATE_FROM, 
 MHCF.EFFECTIVEENDDATE AS DW_DATE_TO, 
 CASE WHEN PARSE_DATE('%Y-%m-%d', '{{ var("VAR_PARAM_GLOBAL_INITIAL_EXTRACT_DATE") }}') > PARSE_DATE('%Y-%m-%d', MHCF.EFFECTIVESTARTDATE)  THEN 1 ELSE 0 END AS DW_INITIAL_RECORD_IND
